...
public class ExtendedFlexTable extends FlexTable implements TableListener {
	...
	public static final int TABLE_WORKFLOW_LIST 		= 0;
	public static final int TABLE_WORKFLOW_VERSIONS 	= 1;
	public static final int TABLE_WORKFLOW_INSTANCES 	= 2;
	
	private int mouseX = 0;
	private int mouseY = 0;
	private int selectedRow = -1;
	private int dataType = TABLE_WORKFLOW_LIST;
		...
		if (selectedRow>0) {
			...
			if (DOM.eventGetType(event) == Event.ONDBLCLICK) {
				...
				case Event.BUTTON_RIGHT:
		            ...
		            switch(dataType) {
		            	case TABLE_WORKFLOW_VERSIONS:
		            		Main.get().centerPanel.workflowPanel.workflowVersionData.showMenu();
		            		break;
		            }
				...
				switch(dataType) {
	            	case TABLE_WORKFLOW_LIST:
	            		String name = getText(selectedRow, 1); 
	    				Main.get().centerPanel.workflowPanel.workflowVersionData.findAllProcessDefinitionVersions(name);
	            		break;
	            	
	            	case TABLE_WORKFLOW_VERSIONS:
	            		double id = new Double(getText(selectedRow,0));
