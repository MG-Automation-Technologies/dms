...
namespace MSOpenKMCore.util
{
    ...
    public class AplicationWatcher
    {
        // Public events
        ...
        void AplicationWatcherProc(object sender, DoWorkEventArgs e)
        {
            int hwndFocus = GetForegroundWindow();
            bool initialFocus = ((m_hwnd == hwndFocus) || (m_hwndOpenKM == hwndFocus));
            ...
            while (!m_Worker.CancellationPending)
            {
                ...
                if ((m_hwnd == hwndFocus) || (m_hwndOpenKM == hwndFocus))
                {
