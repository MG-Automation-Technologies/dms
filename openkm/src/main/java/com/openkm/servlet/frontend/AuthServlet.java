...
public class AuthServlet extends OKMRemoteServiceServlet implements OKMAuthService {
	...
	public void logout() throws OKMException {
		...
		try {
			...
			throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMAuthService, ErrorCode.CAUSE_Repository),
					e.getMessage());
		} catch (DatabaseException e) {
			log.error(e.getMessage(), e);
			throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMAuthService, ErrorCode.CAUSE_Database),
					e.getMessage());
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMAuthService, ErrorCode.CAUSE_General),
					e.getMessage());
		}
	...
	public Map<String, Integer> getGrantedRoles(String nodePath) throws OKMException {
		...
		try {
			...
			log.warn(e.getMessage(), e);
			throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMAuthService, ErrorCode.CAUSE_AccessDenied),
					e.getMessage());
		} catch (RepositoryException e) {
			log.error(e.getMessage(), e);
			throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMAuthService, ErrorCode.CAUSE_Repository),
	...
	public void changeSecurity(String path, Map<String, Integer> addUsers, Map<String, Integer> revokeUsers,
		...
		log.debug("changeSecurity({}, {}, {}, {})", new Object[] { path, addUsers, revokeUsers, addRoles, revokeRoles,recursive });
		updateSessionManager();
		
		try {
			OKMAuth.getInstance().changeSecurity(null, path, addUsers, addRoles, recursive);
		} catch (PathNotFoundException e) {
			log.warn(e.getMessage(), e);
