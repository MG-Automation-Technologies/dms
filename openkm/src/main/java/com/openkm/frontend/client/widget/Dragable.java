						...
						case TREE_ROOT:
							...
							if (clickedTreeItem != null
								...
								if (draggedTreeItem != clickedTreeItem && !isChild) {
									String fldPath = ((GWTFolder) draggedTreeItem.getUserObject()).getPath(); // Folder
																												// actual
																												// path
									final String dstPath = ((GWTFolder) clickedTreeItem.getUserObject()).getPath(); // Destination
																													// path
									TreeItem parentItem = draggedTreeItem.getParentItem(); // The parent of actual item
																							// selected
									
									// Remove the folders and evaluates parent child status
									draggedTreeItem.remove();
									if (parentItem.getChildCount() == 0) {
										((GWTFolder) parentItem.getUserObject()).setHasChilds(false); // Sets not has
																										// folder childs
									}
									
									clickedTreeItem.addItem(draggedTreeItem); // Adds the draggedItem to selected
									((GWTFolder) clickedTreeItem.getUserObject()).setHasChilds(true); // Always sets
																										// that the
																										// actual parent
																										// folder now
																										// has childs
									...
									if (Main.get().mainPanel.desktop.browser.fileBrowser.isFolderSelected()) {
										...
										if (!dstPath.startsWith(fldPath)) {
											...
											if (actualTreeItem.getChildCount() == 0) {
												((GWTFolder) actualTreeItem.getUserObject()).setHasChilds(false); // Sets
																													// not
																													// has
																													// folder
																													// childs
											...
											((GWTFolder) clickedTreeItem.getUserObject()).setHasChilds(true); // Always
																												// sets
																												// that
																												// the
																												// actual
											...
											folderService.move(fldPath, dstPath, new AsyncCallback<Object>() {
												public void onSuccess(Object result) {
													// Sets the folder new path ( parent and itself ) recursive for
													// itself and childs
