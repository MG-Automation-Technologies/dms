...
import es.git.openkm.backend.client.service.OKMFolderServiceAsync;
...
public class FolderSelectTree extends Composite implements TreeListener {
	...
	public void changeView(int view){
		...
		switch (view){
			case FolderSelectPopup.NAVIGATOR_TAXONOMY :
				getRoot();
				break;
				
			case FolderSelectPopup.NAVIGATOR_TEMPLATES :
				getTemplate();
				break;
		}
	}
	 ...
	 * Gets asyncronous root node
	 */
	final AsyncCallback callbackGetTemplate = new AsyncCallback() {
		public void onSuccess(Object result) {
			// Only executes on initalization and the actualItem is root 
			// element on initialization
			//We put the id on root
