...
public class FolderTree extends Composite implements TreeListener, OriginPanel {
	...
	ArrayList<String> tmpAllPathFolder = new ArrayList<String>(); // Used to preserve all node path route of a path
	public int folderAction = ACTION_NONE;  // To control rename and create folder actions
	...
	public FolderTree() {
		...
		rootItem.addStyleName("okm-DisableSelect"); // Disables drag and drop browser text selection
	...
	final AsyncCallback<List<GWTFolder>> callbackGetChilds = new AsyncCallback<List<GWTFolder>>() {
		public void onSuccess(List<GWTFolder> result) {
			...
			for (Iterator<GWTFolder> it = folderList.iterator(); it.hasNext();) {
				...
				// If has no childs directly add values is permited, else evalues
	...
	final AsyncCallback<GWTFolder> callbackGet = new AsyncCallback<GWTFolder>() {
		...
		public void onFailure(Throwable caught) {
			folderAction = ACTION_NONE; // Ensures on error folder action be restores
			Main.get().mainPanel.navigator.status.unsetFlagGet();
	...
	final AsyncCallback<GWTFolder> callbackCreate = new AsyncCallback<GWTFolder>() {
		public void onSuccess(GWTFolder result) {
			actualItem = actualItem.getParentItem(); // Restores the real actualItem
			...
			// Special case when we are creating a folder and selects other tree item before removing tmp folder 
	...
	final AsyncCallback<GWTFolder> callbackRename = new AsyncCallback<GWTFolder>() {
		public void onSuccess(GWTFolder result) {
			...
			path = path.substring(1);		// deletes first character "/" because parent path not has
			oldPath = oldPath.substring(1); // deletes first character "/" because parent path not has
	...
	final AsyncCallback callbackAddSubscription = new AsyncCallback() {
		public void onSuccess(Object result){
			...
			Main.get().mainPanel.topPanel.toolBar.checkToolButtomPermissions((GWTFolder) actualItem.getUserObject(),folderParent,TREE_ROOT);
			evaluesFolderIcon(actualItem);
	...
	final AsyncCallback<Boolean> callbackIsValidDocument = new AsyncCallback<Boolean>() {
		public void onSuccess(Boolean result){			
			if (result.booleanValue()) {
				...
				tmpFldPath = Main.get().fldPath; // Solve problems on refreshing folder and not refreshing file browser
	...
	public void getChilds(String path) {
		...
		Main.get().mainPanel.navigator.status.setFlagChilds();
	}
	
	/**
	 * Refresh the folders on a item node
	 * Used on firstTime loading for personal, trash and templates.
	 * Only loads tree folders not refresh filebrowser
	 * 
	 * @param path The folder path selected to list items
	 */
	 ...
	 * 
	 * @param path The folder subpath selected by user = name
	 */
	public void create(String path) {
		ServiceDefTarget endPoint = (ServiceDefTarget) folderService;
		endPoint.setServiceEntryPoint(Config.OKMFolderService);	
		// On creation the actualItem is tmpFolder and must create from the parent of tmpFolder ( the real actualItem )
		folderService.create(path, ((GWTFolder) actualItem.getParentItem().getUserObject()).getPath(),callbackCreate);
	...
	/**
	 * Deletes a folder, the folder deleted is the actual item and refresh the parent
	 */
	public void delete() {
		ServiceDefTarget endPoint = (ServiceDefTarget) folderService;
		endPoint.setServiceEntryPoint(Config.OKMFolderService);	
	...
	/**
	 * Remove folder from tree after deleted from repository ( normaly deleted on file browser )
	 * 
	...
	public void removeSubscription() {
		...
		notifyService.unsubscribe(((GWTFolder) actualItem.getUserObject()).getPath(),callbackRemoveSubscription);
	}
	...
	private void openDocumentByBrowserURLParam() {
		...
		// Opens folder passed by parameter
		if (Main.get().fldPath!=null && !Main.get().fldPath.equals("") && Main.get().docPath!=null && !Main.get().docPath.equals("")) {
	...
	/**
	 * Refreshing when folder is renamed remotelly
	 * For example on file browser view
	 * 
	 * @param path The tree path ( last value )
	 * @param newFolder The new object value
	 */
	public void renameRenamed(String path, GWTFolder newFolder) {
		...
		while (!found && actualItem.getChildCount() > i) {
			...
			if (((GWTFolder)tmp.getUserObject()).getPath().equals(path)) {
				...
				// Evalues Folder Icon puts name and icon correct values from object
				evaluesFolderIcon(tmp);
				found = true;
				String oldPath = path.substring(1);  	 // deletes first character "/" because parent path not has and replace  not runs properly
				path = newFolder.getPath().substring(1); // deletes first character "/" because parent path not has and replace  not runs properly
	...
	public void openAllPathFolder(String fldId, String docPath) {
		...
		tmpFldPath = actualPath; // Sets the last folder to be opened ( getChilds method needs it for last folder)
		
		// Set the selected document to file browser
		// The browser is only called on the last child node refreshing and restores selected row id
		if (refreshFileBrowser && docPath!=null && !docPath.equals("")) {
			Main.get().mainPanel.browser.fileBrowser.setSelectedRowId(docPath);
		} else {
			Main.get().mainPanel.browser.fileBrowser.setSelectedRowId(""); // We remove the selectedRowID
		...
		if (!tmpAllPathFolder.isEmpty()){
			...
			centerActualItemOnScroll(actualItem); // Centers the actualItem selected if scroll is showed 
			if (refreshFileBrowser) {
		
			// && docPath!=null && !docPath.equals("") 
			// Refreshing false and docPath="" special case parameter path pased on browser uri to open 
			// panels
			Main.get().mainPanel.browser.fileBrowser.refresh(rootPath);
			}
		}
	}
		...
		switch(folderAction){
			case ACTION_CREATE:
				...
				if (!actualItem.equals(item)){
					// Special case when we are creating a folder and selects other tree item
					FolderTextBox folder = (FolderTextBox) tmpFolder.getWidget();
					// Ensures is text writen before create folder on other case eliminates
			...
			default :
				...
				if (actualItem.equals(item)	&& tree.isShowPopUP()) {
					refresh = false;        	// On right click must not refresh browser but must change properties view
					showTabFolderProperties();	// an removes browser selected file or document if any is selected to change  perspective
					...
					if (!actualItem.equals(item)) {
						...
						// When the same node is selected must refresh without mantaining selected filebrowser row
						refresh = false;
						if (!isActualItemRoot()) {
							GWTFolder folderParent = (GWTFolder) actualItem.getParentItem().getUserObject();
							Main.get().mainPanel.topPanel.toolBar.checkToolButtomPermissions((GWTFolder) actualItem.getUserObject(),folderParent,TREE_ROOT);
						} else {
							Main.get().mainPanel.topPanel.toolBar.checkToolButtomPermissions((GWTFolder) actualItem.getUserObject(),folderRoot,TREE_ROOT);
						}
						showTabFolderProperties();  // On this special case refreshing tab folder properties
						Main.get().mainPanel.browser.fileBrowser.deselecSelectedRow();
					}
				}
				break;
	...
	public void refresh(boolean reset) {
		hideMenuPopup();		
		evaluesFolderIcon(actualItem); // Ensures to contemplate any security folder privileges change refresh
		String path = ((GWTFolder) actualItem.getUserObject()).getPath();
		getChilds(path);
		
		// Case not resets always musy show tabfolder properties
		if (!reset) {
			// Case exists a selected row must mantain other case mus show folder properties on tab
	 ...
	 * Set a new tree node
	 * 
	 * @param path Foder Path
	 */
	public void setActiveNode(String path, boolean refreshFileBrowser) {
		// Actually the refresh node only can be called on a subnode on upper version
	...
	/**
	 * Remove temporary folder on creation
	 * Only executed when user closes popup name and
	 */
	...
	/**
	 * Evaluates the special case when creating or renaming a folder, 
	...
	public void securityRefresh(){
		folderAction = ACTION_SECURITY_REFRESH; // To make tab properties refresh after get folder
		get();
	}
	
	/**
	 * Refresh tree iten values when tree folder is changed ( normally on file browser, or by security ).
	 * 
	 * @param folder The new folder values
	 */
	 ...
	 * @param itemToChange The tree item to change the path
	 */
	...
	final AsyncCallback callbackPurge = new AsyncCallback() {
		public void onSuccess(Object result){
			...
			evaluesFolderIcon(actualItem);
			showTabFolderProperties();
			Main.get().mainPanel.navigator.status.unsetFlagPurge();
			refresh(true);
		}
	...
	public void purgeTrash() {
		...
		Main.get().mainPanel.navigator.status.setFlagPurgeTrash();
	}
	 ...
	 * 
	 * @param item The item to show visible
	 */
	private void centerActualItemOnScroll(TreeItem item) {
		Main.get().mainPanel.navigator.scrollTaxonomyPanel.ensureVisible(item);
	}
