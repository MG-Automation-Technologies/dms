...
<body>
  ...
  <c:choose>
    <c:when test="${isAdmin}">
      <c:choose>
        <c:when test="${action == 'create'}"><h1>Create crontab</h1></c:when>
        <c:when test="${action == 'edit'}"><h1>Edit crontab</h1></c:when>
        <c:when test="${action == 'delete'}"><h1>Delete crontab</h1></c:when>
      </c:choose>
      <form action="CronTab" method="post" enctype="multipart/form-data">
        ...
        <table class="form" width="425px">
          ...
          <tr>
            ...
            <td><input class=":email :required :only_on_blur" name="ct_mail" value="${ct.mail}"/></td>
          </tr>
          <tr>
