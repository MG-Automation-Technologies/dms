...
<body>
  ...
  <c:choose>
    <c:when test="${isAdmin}">
      <c:choose>
        <c:when test="${action == 'roleCreate'}"><h1>Create role</h1></c:when>
        <c:when test="${action == 'roleEdit'}"><h1>Edit role</h1></c:when>
        <c:when test="${action == 'roleDelete'}"><h1>Delete role</h1></c:when>
      </c:choose>
      <form action="Auth">
          ...
          <tr>
            ...
            <td width="100%">
              <c:choose>
                ...
                <c:otherwise>
                  <input class=":required :only_on_blur" name="rol_id" value=""/>
                </c:otherwise>
              </c:choose>
            </td>
          </tr>
